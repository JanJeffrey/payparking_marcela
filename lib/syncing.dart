...
class SyncingPage extends StatefulWidget{
  ...
  SyncingPage({Key key, @required this.passData,}) : super(key: key);
...
//        print('ang ctr ani kay $ctr');
//        print('ang length ani kay ${plateData.length}');
...
//      }
     Map dataUser;
     List plateData;
     final response = await http.post(Constants.dbSource + "/${Constants.bertingserver}app_downLoadUser",body:{
       "tohide":"tohide"
...
//print(dataUser['user_details']);
  ...
  Future downloadLocationUser() async{
    ...
    for(int i = 0; i < count; i++) {
      Map dataUser;
      List plateData;
      final response1 = await http.post(Constants.dbSource + "/${Constants.bertingserver}app_downLoadlocation_user", body: {
        "tohide": "tohide"
      });
   ...
   Future downloadManager() async{
    ...
    setState(() {
     ...
     for(int i = 0; i < count; i++) {
       Map dataUser;
       List plateData;
       final response1 = await http.post(Constants.dbSource + "/${Constants.bertingserver}app_downLoadManager", body: {
         "tohide": "tohide"
       });
       dataUser = jsonDecode(response1.body);
       plateData = dataUser['user_details'];
  ...
  Future downloadLocation() async{
    ...
    for(int i = 0; i < count; i++) {
      Map dataUser;
      List plateData;
      final response1 = await http.post(Constants.dbSource + "/${Constants.bertingserver}app_downLoadlocation", body: {
        "tohide": "tohide"
      });
      dataUser = jsonDecode(response1.body);
      plateData = dataUser['user_details'];
   ...
   Future downloadDelinquent() async{
     ...
     for(int i = 0; i < count; i++) {
       Map dataUser;
       List plateData;
       final response1 = await http.post(Constants.dbSource + "/${Constants.bertingserver}app_downloadDelinquent", body: {
         "tohide": "tohide"
       });
       dataUser = jsonDecode(response1.body);
       plateData = dataUser['user_details'];
...
//       );
     ...
     for(int i = 0; i < count; i++) {
       Map dataUser;
       List plateData;
       final response1 = await http.post(Constants.dbSource + "/${Constants.bertingserver}app_downloadDelinquent2", body: {
         "tohide": "tohide"
       });
       dataUser = jsonDecode(response1.body);
       plateData = dataUser['user_details'];
     ...
     showDialog(
       ...
       builder: (BuildContext context) {
         ...
         return CupertinoAlertDialog(
           title: new Text("Syncing successfully"),
